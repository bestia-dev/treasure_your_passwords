...
pub(crate) fn list_tokens_from_vault() -> anyhow::Result<Vec<String>> {
    ...
    for encrypted_text_with_metadata in vec_encrypted_text_with_metadata.iter() {
        let iter_token_name =
            if let Some(iter_token_name) = encrypted_text_with_metadata.token_name.as_ref() {
                iter_token_name
            } else {
                continue;
            };
...
pub(crate) fn store_secret_token_to_vault(token_name: &str) -> anyhow::Result<()> {
    ...
    if !std::fs::exists(private_key_path_struct.get_full_file_path())? {
        ...
        println!(
            r#"{GREEN}ssh-keygen -t ed25519 -f "{private_key_path_struct}" -C "vault for secret tokens"{RESET}"#
        );
        anyhow::bail!("Private key file not found.");
    }
